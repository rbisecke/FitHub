...
- **FK split (intentional, do not change):** Early tables (pre-migration 0024) reference `public.profiles(id)`; tables from 0024 onward reference `auth.users(id)` directly. Both produce identical CASCADE behavior since `profiles.id = auth.users.id`. All future tables: use `REFERENCES auth.users(id) ON DELETE CASCADE`.

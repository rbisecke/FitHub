...
export function DateGroupHeader({ date, count }: DateGroupHeaderProps) {
  ...
  const desktopLabel = dateObj.toLocaleDateString("en-US", {
    weekday: "long",
    month: "short",
    day: "numeric",
    year: "numeric",
  ...
  const mobileLabel = dateObj
    .toLocaleDateString("en-US", { month: "short", day: "numeric" })
    .toUpperCase();

  return (
    <>
      {/* Mobile: compact label + separator */}
      <div className="flex items-center gap-[9px] mb-[11px] mt-[16px] md:hidden">
        <span className="font-data text-[11px] text-[var(--muted-foreground)] uppercase tracking-[0.5px] shrink-0">
          {mobileLabel}
        </span>
        <span className="h-px flex-1 bg-[var(--border)]" />
      </div>

      {/* Desktop: full date + count */}
      <div className="hidden md:flex items-center gap-3 mt-[18px] mb-3 pl-[38px]">
